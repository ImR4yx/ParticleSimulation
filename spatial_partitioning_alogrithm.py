...
def get_grid_coords(p, cell_size):
...
def main():
    ...
    while run:
        ...
        if MainMenu.current_state == MainMenu.MENU:
            ...
            # 2. Populate the grid
            for p in particles:
                key = get_grid_coords(p, CELL_SIZE)
                if key not in grid: grid[key] = []
                grid[key].append(p)
            # 3. Collision Logic
            # Keep track of pairs checked this frame to prevent double-checking
            checked_pairs = set() 
            for key, cell_particles in grid.items():
                x, y = key
                # Check only current cell and 4 "forward" neighbors (prevents double checking)
                # This is a common optimization to avoid the full 9-cell check
                for dx in range(-1, 2):
                    for dy in range(-1, 2):
                        ...
                        if neighbor_key in grid:
                            for p1 in cell_particles:
                                for p2 in grid[neighbor_key]:
                                    if p1 != p2:
                                        # Use object ID to create a unique pair key
